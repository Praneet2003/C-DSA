...
// Function to calculate hash using Mid-Square Hashing
int midSquareHash(int key, int tableSize, int r) {
    long long square = (long long)key * key; // Square the key
    int middleDigit = (square / 10) % 10;   // Extract the middle digit (1 digit, as per r = 1)
    return middleDigit % tableSize;         // Return the hash index
}
